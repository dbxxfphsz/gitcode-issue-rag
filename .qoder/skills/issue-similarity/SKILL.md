...
description: 当用户需要扫描 GitCode Issue 的相似历史问题、把 Issue 加入知识库、定时扫描并评论新 Issue、或基于历史 Issue 生成 FAQ 文档时使用。触发语如"扫描这个 Issue 看有没有类似问题""把这个 Issue 加入知识库""每天定时扫描新 Issue 并评论相似问题""基于历史 Issue 生成 FAQ""查看知识库状态"。
---
...
- 验证阶段建议优先使用 `--mode report`，避免真实评论污染线上 Issue
